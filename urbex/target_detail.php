...
<script>
    ...
    async function loadLocationDetails(locationId) {
        try {
            const response = await fetch('utilities/retrieve_target_locations.php');
    ...
    async function loadImages(locationId) {
        return new Promise((resolve, reject) => {
            const scriptUrl = 'utilities/get_target_images.php';
            $.ajax({
                url: scriptUrl,
                method: 'GET',
    ...
    async function displayLocationDetails() {
        ...
        const locationId = parseInt(locationId_str, 10);
        if (isNaN(locationId)) {
        ...
        if (location) {
            locationDetailsContainer.innerHTML += `
                ...
                <img loading="lazy" src="images/targets/${location.id}/${location.titleImage}" alt="Location Image">
    ...
    function toggleGallery() {
        ...
        const button = document.querySelector(".dropdown-btn:nth-of-type(2)"); // Select the second button
